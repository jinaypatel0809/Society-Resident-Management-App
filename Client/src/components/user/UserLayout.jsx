...
const NAV_ITEMS = [
  ...
  { label: "Notices",         icon: Bell,                 section: "notices"       },
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium
                  ...
                  ${isActive
                    ? "bg-resident/20 text-resident-accent"
                    : "text-white/60 hover:text-white hover:bg-white/8"
                  }`}
              >
                <Icon size={17} className="flex-shrink-0" />
                <span className="flex-1">{label}</span>
                {isActive && <ChevronRight size={14} className="text-resident-accent/60" />}
              </button>
            );
          })}
        </nav>
        ...
        <header className="bg-white border-b border-gray-100 px-5 py-3.5 flex items-center gap-4 sticky top-0 z-10">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs hidden sm:block">
              {user?.societyName || "Society"} · Resident Portal
            </p>
          </div>
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-[10px]
                  font-bold rounded-full min-w-[16px] h-4 px-1 flex items-center justify-center">
